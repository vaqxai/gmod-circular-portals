...
if SERVER then
	util.AddNetworkString("HLSPRTL_LDP")
	util.AddNetworkString("HLSPRTL_CAS")
end
...
properties.Add("portal_persist", {
	...
	Filter = function(self, ent, player)
		...
		if (SERVER and not IsValid(ent:GetOther())) then
			player:ChatPrint("This portal does not have an exit!")

			return false
		end

		player:ChatPrint("You have successfully tried to save two portals!")
	...
	MenuOpen = function(self, option, ent, trace)
		...
		option = subMenu:AddOption("None", function()
...
if (SERVER) then
	...
	net.Receive("HLSPRTL_LDP", function()
		SavePortals()
	end)
	...
	function SavePortals()
		...
		for k, v in pairs(ents.FindByClass("helios_gateway")) do
		...
		if (buffer and table.Count(buffer) > 0) then
			...
			file.CreateDir("heliosgates")
			file.Write("heliosgates/" .. game.GetMap() .. ".txt", JSON)
			print("[heliosGATES] Saved two portals on " .. game.GetMap())
		else
			file.Delete("heliosgates/" .. game.GetMap() .. ".txt")
		end
	...
	function LoadPortals()
		if (not file.Exists("heliosgates/" .. game.GetMap() .. ".txt", "DATA")) then return end
		local buffer = file.Read("heliosgates/" .. game.GetMap() .. ".txt", "DATA")
		...
		if (buffer and buffer:len() > 1) then
			...
			if (portals) then
				for k, info in pairs(portals) do
					...
					local portal1 = ents.Create("helios_gateway")
					local portal2 = ents.Create("helios_gateway")
					...
					if (info.mode == 1 or info.enabled) then
						portal1:Enable()
						portal2:Enable()
						timer.Simple(5 * 0.75,function()
							portal1:SetFinishedAnim(true)
							portal2:SetFinishedAnim(true)
						end)
	...
	timer.Create("helios_SavePortals", 180, 0, function()
		local win, msg = pcall(SavePortals)

		if (not win) then
			ErrorNoHalt("[heliosGATES] Something went wrong when saving portals!\n" .. msg)
		end
	end)

	hook.Add("ShutDown", "helios_SavePortals", function()
		local win, msg = pcall(SavePortals)
	...
	hook.Add("InitPostEntity", "helios_LoadPortals", function()
		timer.Simple(5, function()
			local win, msg = pcall(LoadPortals)

			if (not win) then
				ErrorNoHalt("[heliosGATES] Something went wrong when loading portals!\n" .. msg)
			end
		end)
	end)

	hook.Add("PostCleanupMap", "helios_LoadPortals_Cleanup", function()
		timer.Simple(5, function()
			local win, msg = pcall(LoadPortals)
